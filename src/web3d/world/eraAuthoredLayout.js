...
function cloneBlock(def, owner, defaultRgb) {
  return {
    ...
    ownerType: 'space',
    ownerId: owner.id,
    ownerLabel: owner.label || owner.id,
    spaceId: def.spaceId || owner.id,
    spaceLabel: def.spaceLabel || owner.label || owner.id,
...
function cloneCheckpoint(def, owner, index) {
  return {
    ...
    anchorId: def.anchorId || def.id || makeId(owner.id, `anchor_${index}`),
    ownerId: owner.id,
    ownerLabel: owner.label || owner.id,
    spaceId: owner.id,
    spaceLabel: owner.label || owner.id,
    x: resolveCenteredAxis(owner.x, def.x, def.offsetX),
