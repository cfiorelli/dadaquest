...
- **Global scope**: `/Users/lolm/dadaquest/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/lolm/dadaquest/.claude/notes/`)
...
2. Glob `/Users/lolm/dadaquest/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/lolm/dadaquest/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/lolm/dadaquest/.claude/notes/` — works in any directory
